...
func (iter *iterAbb[K, V]) apilarIzquierdos(nodo *nodoAbb[K, V], desde *K) {
	actual := nodo
	for actual != nil {
		// Si hay un limite inferior y el nodo es menor, no lo apilamos
		if desde != nil && iter.cmp(actual.clave, *desde) < 0 {
			actual = actual.derecho
			continue
		}
		// Si hay limite superior y el nodo es mayor, no lo apilamos
		if iter.hasta != nil && iter.cmp(actual.clave, *iter.hasta) > 0 {
